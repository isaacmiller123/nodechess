...
fold over the covered segment range from that prior snapshot, which makes it **incrementally
verifiable in one step**, and (c) carry **M-of-N cosignatures from distinct eligible witnesses** (§4). A checkpoint
...
- **Tier 2: the only *anticheat* ban trigger** (§9 has other ban types). Deep fixed-node
  ...
  regardless, so colluders can't suppress), or a stranger later. **Aggregation, not gotchas**:
...
- **C-12** On the reconstruction floor (owner gone, <`K_rec` shard rows, no chain linkage to vet a
  ...
  indistinguishable from the legitimate cold-root flow. Honoring it is required so a device-revoked
  key cannot forge (§0), and it can transiently hide an honest device's content. The
...
until A-final. The content plane (puzzle DB, curriculum, famous, personas, all *content and not
account data*) stays conventionally served, out of scope. The operator's Fastify node evolves into the
...
Irwin (statistical cheat detection). The novel part is the composition: entangled personal chains +
deterministic-committee witnesses + self-jailing clients + trust-width matchmaking + a
bit-reproducible judge.