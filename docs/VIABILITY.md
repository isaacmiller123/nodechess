...
> **Status 2026-07-27: not cleared, and the web target shipped anyway.** The static web build is deployable today (docs/DEPLOY-WEB.md) while `resources/personas/personas.json` still carries 24 personas named after real players, including living ones (Carlsen, Nakamura, Caruana, Ding, Gukesh, Polgar, Anand, Kramnik, Rozman), which §4 item 1 says to delete or license before any public launch. Recorded as open, not waived: no decision to defer or narrow the persona set has been taken.
