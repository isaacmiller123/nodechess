...
| rematchOffer / rematchDecline | n/a | symmetric for both roles (MP-07/L11). |
...
sleep, since a sleeping host charges no one). `Date.now()` only for absolute logging. Flag watchdog
...
tick gap since last fire > 2× cadence, we were suspended, so reset lastPeerMsgAt = now, ping, and
...
- `msg.send(...).catch(...)` → new listener `onSendError(err)`; the session treats it like
...
Plain module singleton (NO React imports, so it is testable in bare node) + a separate
...
optimistically, ROLLS BACK if `mp.sendMove` returns ok:false, per D6; blocked while peerAway/over),
...
this also protects the lobby/hosting phase which holds no WebRTC connection yet, per L4).
